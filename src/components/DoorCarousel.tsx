...
import { Door, ContactForm, BusinessInfo, Category } from '../types';
...
const AdminPanel: React.FC = () => {
  ...
  const { data: categories, loading: categoriesLoading, addItem: addCategory, updateItem: updateCategory, deleteItem: deleteCategory } = useFirestore<Category>('categories');
  ...
  const tabs = [
    ...
    { id: 'categories', name: 'ניהול קטגוריות', icon: Tag },
...
}> = ({ doors, loading, categories, onAdd, onUpdate, onDelete }) => {
...
const CategoriesManager: React.FC<{
  ...
  onDelete: (id: string) => void;
}> = ({ categories, loading, onAdd, onUpdate, onDelete }) => {
  const [showAddForm, setShowAddForm] = useState(false);
  ...
  const handleDelete = async (categoryId: string, categoryName: string) => {
    if (window.confirm(`האם אתה בטוח שברצונך למחוק את הקטגוריה "${categoryName}"?`)) {
      try {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold">ניהול קטגוריות ({categories.length} קטגוריות)</h2>
        <button
          onClick={() => setShowAddForm(true)}
          className="bg-orange-800 text-white px-4 py-2 rounded-lg flex items-center space-x-2 space-x-reverse hover:bg-orange-900"
        >
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleAdd}>
              ...
              <div className="flex gap-3">
                <button
                  type="submit"
                  className="flex-1 bg-orange-800 text-white py-2 px-4 rounded-lg hover:bg-orange-900"
                >
                  הוסף
                </button>
                <button
